...
def pretty_country(name: str) -> str:
    """Normaliza nombres para display: quita slugs, y aplica capitalización española."""
    if not name:
        return name
    s = str(name).strip()
    if not s:
    ...
    for i, w in enumerate(parts):
        ...
        if i > 0 and wl in small:
            ...
            # capitaliza primera letra, conserva resto en minúscula
...
def build_points(df: pd.DataFrame, do_ocr: bool, ocr_lang: str) -> tuple[list[dict], dict]:
    ...
    for _, row in df.iterrows():
        ...
        # País en español y con capitalización limpia
        cand = pais_es or country_to_es(pais_dash_raw) or country_to_es(pais_iso2) or country_to_es(pais_raw) or pais_dash_raw or pais_raw
